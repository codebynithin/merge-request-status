...
import { LogOut, RefreshCw, GitMerge, PanelLeftClose, PanelLeftOpen } from 'lucide-react';

const STATES = ['opened', 'merged', 'closed', 'all'] as const;
type StateFilter = (typeof STATES)[number];

export default function App() {
  const [token, setToken] = useState<string>(() => localStorage.getItem('gitlab_token') || '');
  ...
  const loadMRs = async (project: GitLabProject, state: StateFilter = mrState) => {
    setLoadingMRs(true);
    setError('');
    try {
        ...
        <div className=" mx-auto px-5 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2.5">
            ...
            <div className="leading-tight">
              <h1 className="font-semibold tracking-tight">Merge Request Status</h1>
              <div className="text-[11px] text-slate-500">Hierarchical MR viewer for GitLab</div>
            </div>
          </div>
          <div className="flex items-center gap-2 text-sm">
            {me && (
              <div className="hidden sm:flex items-center gap-2 px-2.5 py-1 rounded-full border border-slate-800 bg-slate-900/60">
                {me.avatar_url && (
                  <img src={me.avatar_url} alt="" className="w-5 h-5 rounded-full" />
                )}
      ...
      >
        <aside
          className={`panel py-3 px-1 flex flex-col sticky top-[4.25rem] self-start max-h-[calc(100vh-5.5rem)] ${
            sidebarCollapsed ? '' : 'col-span-12 md:col-span-4 lg:col-span-3 p-3.5'
          }`}
        >
          <div
        ...
        >
          {!selectedProject ? (
            <div className="flex-1 flex flex-col items-center justify-center text-slate-500 gap-2">
              ...
              <div className="text-sm">Select a repository to view its merge requests.</div>
            </div>
          ) : (
            <>
              <div className="flex items-start justify-between mb-4 gap-3 flex-wrap">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    title="Refresh MRs"
                  >
                    <RefreshCw className={`w-4 h-4 ${loadingMRs ? 'animate-spin' : ''}`} />
                  </button>
                </div>
              </div>
