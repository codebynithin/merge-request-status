...
export default function TokenGate({ onSave }: Props) {
  ...
  const [baseUrl, setBaseUrl] = useState<string>(() => getGitlabBaseUrl() || DEFAULT_GITLAB_URL);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6">
      <form onSubmit={submit} className="w-full max-w-md panel p-7 animate-fade-in">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <div className="text-xs text-slate-500">Stacked MRs, visualized.</div>
          </div>
        </div>
        ...
        <p className="text-slate-400 text-sm leading-relaxed mt-4 mb-6">
          ...
          . Your credentials are stored only in this browser and sent directly to GitLab.
        </p>
        ...
        <p className="mt-4 text-[11px] text-slate-500 text-center">
          Create a token at <span className="font-mono">User Settings → Access Tokens</span>.
        </p>
      </form>
    </div>
  );
